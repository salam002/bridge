...
def sign_message(challenge, filename="secret_key.txt"):
    ...
    recovered = eth_account.Account.recover_message(message, signature=signed_message.signature.hex())
    




    assert (recovered == eth_addr), f"Failed to sign message properly — expected {eth_addr}, got {recovered}"
